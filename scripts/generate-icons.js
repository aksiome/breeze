  ...
  return Object.fromEntries(
    Object.entries(simpleIcons)
      ...
      .map(([name, icon]) => [
        ...
        { "24": `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="${icon.path}"/></svg>` }
      ])
  );
}
...
function makeOcticonSvg(icon, size) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${height}" height="${height}" viewBox="0 0 ${height} ${height}" fill="currentColor">${path}</svg>`;
}
